...
1. Кладеш аудіо/відео файли в папку `input/` і натискаєш **Розпізнати мовлення**.
...
- **Windows 10/11.** Лаунчери `.bat` (`setup.bat`, `run_gui.bat`) орієнтовані на
  Windows (сам Python-код кросплатформний).
...
- **Python-залежності** з `requirements.txt` (faster-whisper, pyannote.audio,
  torch, numpy, huggingface_hub, PySide6).
...
і ставить CUDA-збірку torch за наявності, і готує `config.env`. Завершується
запуском самого MeetingScribe.

Під час першого запуску, якщо модель розпізнавання ще не встановлена, застосунок
запитає перед завантаженням - нічого не відбувається тихо, і рекомендований
варіант вже позначено. У комплекті є короткий демо-запис (`samples/demo.wav`),
тож є що спробувати одразу, без запису чогось свого.
...
2. Встав його на вкладці **Settings > General** застосунку й натисни "Save
...
1. Запусти застосунок (`run_gui.bat`, або він відкриється сам наприкінці `setup.bat`).
2. Поклади файли в папку `input/`, або скористайся **Додати файли...** в застосунку.
