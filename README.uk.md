...
Створено для записів зустрічей і розмов, переважно **українською** та
**російською**, з автоматичним визначенням мови.
...
- Запис «на льоту»: назви невідомого спікера й збережи його відбиток
  ...
  пропустити.
- Автоматичне визначення мови (налаштовано на змішану укр/рос).
...
- Windows 10/11 (лаунчери `.bat` орієнтовані на Windows; самі Python-скрипти
  кросплатформні).
- Python 3.10+ (`setup.bat` встановить Python 3.12, якщо його немає).
- [ffmpeg](https://ffmpeg.org/) (встановлюється автоматично через `setup.bat`).
- Python-залежності з `requirements.txt` (faster-whisper, pyannote.audio, torch,
  numpy, huggingface_hub, ffmpeg-python).
- Акаунт [HuggingFace](https://huggingface.co/) і токен доступу **для
  діаризації** (опційно - без нього інструмент працює як звичайна транскрипція).
- Опційно: відеокарта NVIDIA з CUDA для швидшої обробки (автовизначення;
  `setup.bat` встановить CUDA-збірку torch, якщо знайде GPU).
